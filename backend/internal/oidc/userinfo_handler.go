...
func (h *userInfoHandler) userInfo(c *gin.Context) {
	...
	// Resolve the OidcClient from the access request so claim remappings can be applied
	var oidcClient *model.OidcClient
	if c, ok := accessRequest.GetClient().(Client); ok {
		oidcClient = &c.OidcClient
	}

	claims, err := h.claimsService.GetUserClaims(ctx, session.GetSubject(), accessRequest.GetGrantedScopes(), oidcClient)
	if err != nil {
		_ = c.Error(err)
		return
	}
